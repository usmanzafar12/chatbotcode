...
#     language_model = load_lm('bigrams.pkl')
    ...
    for n, m in enumerate(text):
        if m != "<s>" and m != "</s>":
            if not d.check(m):
#                 print "\nError word is:" print m
                lm_wordlist = eng_word_candidates(text[n-1])
                
#   #              lm_wordlist = eng_word_candidates(text[n-1], language_model)
                lm_wordlist = [x.lower() for x in lm_wordlist]
#                 print "\nLanguage Model Suggested list for the word %s:" %text[n-1] print lm_wordlist
                med_words = d.suggest(m)
                med_words = [x.lower() for x in med_words]
                dmeta_words = dmeta_wordlist(m)
#                 print "\nSuggested list by Enchant Library:" print med_words print "\nSuggested list by Double Metaphone:" print dmeta_words
...
#                 print "\nFirst LIST OF MATCHED WORDS" print match print "\nSecond List of Matched words: " print match2
                try:
...
#                             print "Third match is: " print list(match3)
